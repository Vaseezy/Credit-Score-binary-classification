...
def barplot_balance(data: pd.DataFrame, feature: str) -> None:
    """
    Визуализация баланса классов признака в процентах
    ...
    feature : str
        Название категориального признака для анализа баланса
        
    Returns:
    --------
    None
        Отображает столбчатую диаграмму с процентным распределением значений
    """
    plt.figure(figsize=(15, 8))
    
    # Рассчитываем процентное соотношение классов
    feature_counts = data[feature].value_counts(normalize=True) * 100
    
    # Строим график с учетом нового синтаксиса seaborn
    ax = sns.barplot(
        ...
        palette='pastel',
        legend=False,
        dodge=False  # Убираем раздвигание столбцов
    )
    
    # Настройки оформления
    plt.title(f'Распределение признака "{feature}"', pad=20, fontsize=14)
    plt.ylabel('Процентное соотношение (%)', labelpad=10)
    plt.xlabel(f'Категории признака {feature}', labelpad=10)
    ...
    plt.ylim(0, min(100, feature_counts.max() * 1.15))
...
def multiclass_kde_hue(data: pd.DataFrame, target: str, feature: str):
    """
    Визуализация распределения числового признака по классам целевой переменной
    ...
    data : pd.DataFrame
        Исходный датафрейм
    target : str
        Название целевой переменной (категориальная)
    feature : str
        Название анализируемого числового признака
    """
    plt.figure(figsize=(15, 8))
    
    # Создаем отдельный график для каждого класса
    for class_value in data[target].unique():
        subset = data[data[target] == class_value][feature]
        sns.kdeplot(subset, label=f'{target} = {class_value}', fill=True)
    
    # Настройка оформления
    plt.title(f'Distribution of {feature} by {target}', pad=20)
    plt.xlabel(feature)
    plt.ylabel('Density')
    plt.legend(title=target)
    plt.show()

def multiclass_displot_hue(data: pd.DataFrame, target: str, feature: str):
    # Создаем фигуру и оси
    plt.subplots(figsize=(15, 8))
    for i in data[target].value_counts().index.tolist():
        # Рисуем распределения дохода по n группам на осях
        sns.displot(data[data[target] == i][feature], hist=False, kde=True, label=f'{target} = {i}')
